// Helper: image with fallback (renders raw SVG)
function icon(src){
...
// Catalog mapped to your exact filenames in /img/icons/foodandbevs/
const items = [
...
function renderBevs(){
  ...
  items.forEach(it => {
    ...
    // Add to cart on tap/click
    card.addEventListener('click', () => Cart.addItem({
      id: it.id,
      sku: it.sku,
      name: it.name,              // kept for invoice line item
