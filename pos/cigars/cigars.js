...
(() => {
  ...
  function pushRecentBrand(name) {
    ...
    const next = [
      target,
      ...getRecentBrands().filter(
        (b) => norm(b).toLowerCase() !== target.toLowerCase()
      ),
  ...
  function rowMatchesFilters(row, g) {
    ...
    const shade = norm(
      getField(row, ["Wrapper Shade", "WrapperShade", "wrapperShade", "shade"])
    );
    ...
    if (q) {
      const cigarName = norm(
        getField(row, ["Cigar", "Cigar Name", "Name", "cigar", "cigar_name"])
      );
  ...
  function buildBrandSummary(rows) {
    ...
    for (const r of rows) {
      const brand =
        norm(getField(r, ["Brand", "brand", "Brand aka", "brand_aka"])) || "Unknown";
  ...
  function orderWrapperShades(values) {
    ...
    WRAPPER_SHADE_ORDER.forEach((item) => {
  ...
  function ensureInjectedStyles() {
    ...
    style.textContent = `
      ...
      .fm.fm--tabs .fm__cuban-row{ display:none; }
      ...
      .fm__range-input{
        ...
        height:76px;
        border-radius:999px;
        border:2px solid rgba(15,26,44,.10);
        background:#fff;
        color:#5aa9e8;
        font-size:34px;
        font-weight:800;
        text-align:center;
        outline:none;
        box-shadow:0 8px 22px rgba(15,26,44,.05);
      }
      ...
      .fm__range-sliders input[type="range"]::-webkit-slider-thumb{
        ...
        background:#5aa9e8;
        border:0;
        box-shadow:0 8px 18px rgba(90,169,232,.35);
      }

      .fm__range-sliders input[type="range"]::-moz-range-track{
        height:10px;
        border-radius:999px;
        background:#e8e8eb;
      }

      .fm__range-sliders input[type="range"]::-moz-range-thumb{
        pointer-events:auto;
        width:44px;
        height:44px;
        border-radius:50%;
  ...
  function renderRingRangeFilter(list) {
    ...
    list.innerHTML = `
      <div class="fm__range-ui">
        <div class="fm__range-title">Ring Gauge</div>
        <div class="fm__range-sub">Only show cigars within this ring range.</div>
        ...
        <div class="fm__range-note">
          Showing ring sizes from <strong id="ring-min-label">${currentMin}</strong>
          to <strong id="ring-max-label">${currentMax}</strong>
        </div>
      </div>
    `;
    ...
    list.innerHTML = filtered
      .map((v) => {
        ...
        const icon = iconSrc
          ? `
            <div class="${iconClass}">
              <img src="${escapeHtml(iconSrc)}" alt="" loading="lazy" decoding="async" onerror="this.style.display='none';" />
            </div>
          `
  ...
  function openModal() {
    ...
    window.setTimeout(() => {
      if (state.activeKey !== "ring") $("#fm-search-inline", modalRoot)?.focus();
    }, 60);
  }
